...
void apply_stencil(const ACC<double> &A, ACC<double> &Anew, double *error) {
  ...
  *error = fmax( *error, fabs(Anew(0,0)-A(0,0)));
        ...
        // # if list(assignment_node.get_children())[0].kind == CursorKind.UNARY_OPERATOR or list(assignment_node.get_children())[1].kind == CursorKind.UNARY_OPERATOR:
        // #     print("FAILURE MODE 1")
        // #     self.debug_tree(assignment_node)
        // #     exit(0)