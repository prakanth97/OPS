...
// Auto-generated at 2025-02-12 14:37:05.903874 by ops-translator
...
void ops_par_loop_set_zero(char const *, ops_block, int, int*, ops_arg);

void ops_par_loop_left_bndcon(char const *, ops_block, int, int*, ops_arg, ops_arg);

void ops_par_loop_right_bndcon(char const *, ops_block, int, int*, ops_arg, ops_arg);

void ops_par_loop_apply_stencil(char const *, ops_block, int, int*, ops_arg, ops_arg, ops_arg);

void ops_par_loop_copy(char const *, ops_block, int, int*, ops_arg, ops_arg);
...
int main(int argc, const char** argv)
{
  ...
  ops_init(argc, argv,1); // No diagnostics
	ops_init_backend();
  ...
  ops_decl_const2("pi",1,"double",&pi);

  ops_partition("");

  // set boundary conditions
  int bottom_range[] = {-1, imax+1, -1, 0};
  ops_par_loop_set_zero("set zero", block, 2, bottom_range,
      ops_arg_dat(d_A, 1, S2D_00, "double", OPS_WRITE));

  int top_range[] = {-1, imax+1, jmax, jmax+1};
  ops_par_loop_set_zero("set zero", block, 2, top_range,
      ops_arg_dat(d_A, 1, S2D_00, "double", OPS_WRITE));

  int left_range[] = {-1, 0, -1, jmax+1};
  ops_par_loop_left_bndcon("left bndcon", block, 2, left_range,
      ops_arg_dat(d_A, 1, S2D_00, "double", OPS_WRITE),
      ops_arg_idx());

  int right_range[] = {imax, imax+1, -1, jmax+1};
  ops_par_loop_right_bndcon("right bndcon", block, 2, right_range,
      ops_arg_dat(d_A, 1, S2D_00, "double", OPS_WRITE),
      ops_arg_idx());
  ...
  while ( error > tol && iter < iter_max )
  {
    ...
    ops_par_loop_apply_stencil("apply stencil", block, 2, interior_range,
